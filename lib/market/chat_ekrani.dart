...
class _OtoDnaChatScreenState extends State<OtoDnaChatScreen> {
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(widget.saticiAdi, style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                    SizedBox(height: 2),
                        ...
                        builder: (context, snapshot) {
                          if (snapshot.hasData && snapshot.data!.exists) {
                            ...
                            if (karaListedeMi) {
                              return Row(
                                children: [
                                  Icon(Icons.warning_amber_rounded, color: SiberTema.kanKirmizi, size: 12),
                                  SizedBox(width: 4),
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Icon(Icons.shield_outlined, color: _neonGreen, size: 20),
              ),
              SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(widget.urunAdi, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
                    SizedBox(height: 2),
                    Text(
              ...
              Text(widget.urunFiyati, style: TextStyle(color: _neonGreen, fontWeight: FontWeight.bold, fontSize: 14)),
            ],
          ),
        );
      },
  ...
  Widget _buildMesajListesi() {
    return StreamBuilder<QuerySnapshot>(
      stream: _db
          ...
          .snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) return Center(child: CircularProgressIndicator(color: _neonGreen));
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) return Center(child: Text("Siber Sohbet Başladı", style: TextStyle(color: Colors.white38)));
            ...
            if (isSistem) {
              return Center(
                child: Container(
                  ...
                  child: Text(msj["metin"] ?? "", style: TextStyle(color: SiberTema.siberGold, fontSize: 10, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                ),
              );
            }
        ...
        children: [
          Expanded(
            child: OutlinedButton.icon(
              ...
              label: Text("Foto Yükle", style: TextStyle(fontSize: 12)),
          ...
          Expanded(
            child: ElevatedButton.icon(
              ...
              label: Text("Teslim Aldım", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
              onPressed: () async {
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Icon(Icons.send, color: _neonGreen, size: 20),
            ),
          )
        ],
      ),
  ...
  Future<void> _mesajGonder() async {
    ...
    if (ihlalVar) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("🚨 SİBER İHLAL: İletişim bilgisi veya IBAN paylaşılamaz!", style: TextStyle(fontWeight: FontWeight.bold)), backgroundColor: SiberTema.kanKirmizi));
