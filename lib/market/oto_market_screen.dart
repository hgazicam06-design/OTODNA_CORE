...
class _OtoMarketScreenState extends State<OtoMarketScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildSaticiRozeti(int puan, String saticiAdi) {
    ...
    if (puan >= 5) { rozetRengi = siberGold; rozetMetni = "Premium Onaylı"; }
      ...
      return Row(children: [Icon(Icons.star_border_rounded, color: Colors.white38, size: 14), SizedBox(width: 6), Text(saticiAdi.toUpperCase(), style: TextStyle(color: Colors.white54, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1, fontFamily: 'Avenir'))]);
    }
    else {
      return Row(
        children: [
          Container(padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: SiberTema.kanKirmizi.withOpacity(0.1), border: Border.all(color: SiberTema.kanKirmizi.withOpacity(0.5)), borderRadius: BorderRadius.circular(6)), child: Row(children: [Icon(Icons.gpp_bad_outlined, color: SiberTema.kanKirmizi, size: 12), SizedBox(width: 4), Text("KARA LİSTE", style: TextStyle(color: SiberTema.kanKirmizi, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1))])),
          SizedBox(width: 8),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        icon: Icon(Icons.add_a_photo_outlined, color: Colors.black, size: 20),
        label: Text("İLAN VER", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 1, fontFamily: 'Avenir')),
      ),
    );
  }
          ...
          child: Row(
            ...
            children: [
              GestureDetector(
                ...
                child: Icon(Icons.arrow_back_ios_new, color: primaryCyan, size: 20),
              ),
              Text('S İ B E R   P A Z A R', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 14, letterSpacing: 4, fontFamily: 'Avenir')),
              Icon(Icons.shopping_cart_outlined, color: primaryCyan, size: 22),
            ],
          ),
        ),
      ),
        ...
        child: BackdropFilter(
          ...
          child: Container(
            ...
            child: TabBar(
              ...
              labelStyle: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, letterSpacing: 0.5, fontFamily: 'Avenir'),
              tabs: [Tab(text: "Parça Pazarı"), Tab(text: "Oto Galeri")],
            ),
          ),
        ),
      ),
        ...
        Padding(
          ...
          child: Column(
            children: [
              _buildAiAramaKutusu(),
              SizedBox(height: 16),
              Container(padding: EdgeInsets.all(16), decoration: BoxDecoration(color: primaryCyan.withOpacity(0.05), borderRadius: BorderRadius.circular(16), border: Border.all(color: primaryCyan.withOpacity(0.2))), child: Row(children: [Icon(Icons.shield_outlined, color: primaryCyan, size: 20), SizedBox(width: 12), Expanded(child: Text("Bireysel ve Kurumsal tüm satıcılar OtoDNA güvencesindedir.", style: TextStyle(color: Colors.white54, fontSize: 10, fontWeight: FontWeight.bold, height: 1.4, fontFamily: 'Avenir')))]))
            ],
          ),
        ),
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              if (!snapshot.hasData || snapshot.data!.docs.isEmpty) return Center(child: Text("Siber ağda parça bulunmuyor.", style: TextStyle(color: Colors.white38, fontWeight: FontWeight.bold, fontFamily: 'Avenir')));
  ...
  Widget _buildOtoGaleriSekmesi() {
    return Column(
      children: [
        Padding(
          padding: EdgeInsets.symmetric(horizontal: 24, vertical: 8),
          child: Container(padding: EdgeInsets.all(16), decoration: BoxDecoration(color: siberGold.withOpacity(0.05), borderRadius: BorderRadius.circular(16), border: Border.all(color: siberGold.withOpacity(0.2))), child: Row(children: [Icon(Icons.diamond_outlined, color: siberGold, size: 20), SizedBox(width: 12), Expanded(child: Text("OtoDNA Onaylı (VIP) Araçlar Kuantum algoritmasıyla üstte gösterilir.", style: TextStyle(color: Colors.white54, fontSize: 10, fontWeight: FontWeight.bold, height: 1.4, fontFamily: 'Avenir')))])),
        ),
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) return Center(child: CircularProgressIndicator(color: primaryCyan));
              if (!snapshot.hasData || snapshot.data!.docs.isEmpty) return Center(child: Text("Siber galeride araç bulunmuyor.", style: TextStyle(color: Colors.white38, fontWeight: FontWeight.bold, fontFamily: 'Avenir')));
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          child: TextField(
            ...
            style: TextStyle(color: Colors.white, fontSize: 14, fontFamily: 'Avenir'),
            decoration: InputDecoration(
              ...
              suffixIcon: IconButton(icon: Icon(Icons.document_scanner_outlined, color: primaryCyan, size: 20), onPressed: () {}),
    ...
    return GestureDetector(
      onTap: () {
        if (!isKaraListe) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('AĞ UYARISI: Bu satıcı Kara Liste\'dedir.', style: TextStyle(color: Colors.white)), backgroundColor: SiberTema.kanKirmizi));
        }
      },
      child: Opacity(
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(color: Colors.black, borderRadius: BorderRadius.vertical(top: Radius.circular(24)), image: DecorationImage(image: NetworkImage(resimUrl), fit: BoxFit.cover)),
              ),
              Padding(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Stack(
            children: [
              Container(height: 200, decoration: BoxDecoration(color: Colors.black, borderRadius: BorderRadius.vertical(top: Radius.circular(32)), image: DecorationImage(image: NetworkImage(resimUrl), fit: BoxFit.cover))),
              if (isOtoDnaOnayli)
                Positioned(top: 16, left: 16, child: Container(padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: Colors.black.withOpacity(0.9), borderRadius: BorderRadius.circular(12), border: Border.all(color: primaryCyan.withOpacity(0.5))), child: Row(children: [Icon(Icons.qr_code_scanner_outlined, color: primaryCyan, size: 16), SizedBox(width: 8), Text("OtoDNA ONAYLI", style: TextStyle(color: primaryCyan, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1, fontFamily: 'Avenir'))]))),
            ],
          ),
          Padding(
            padding: EdgeInsets.all(24.0),
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Expanded(child: Text(aracAdi, style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900, letterSpacing: 0.5, fontFamily: 'Avenir'), overflow: TextOverflow.ellipsis, maxLines: 2)),
                    SizedBox(width: 16),
                    Text(fiyat, style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900, letterSpacing: -1, fontFamily: 'Avenir')),
                  ],
                ),
                SizedBox(height: 12),
                Text(detay, style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
  ...
  void _siberIlanVerDialog() {
    showModalBottomSheet(
      context: context, backgroundColor: Colors.black.withOpacity(0.9), isScrollControlled: true, shape: RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(32))),
      builder: (ctx) {
        return Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("SİBER İLAN TERMİNALİ", style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w900, letterSpacing: 1, fontFamily: 'Avenir')), IconButton(icon: Icon(Icons.close, color: Colors.white54), onPressed: () => Navigator.pop(ctx))]),
              SizedBox(height: 24),
              ...
              _buildIlanSecenekKarti("Otomobil & Taşıt (Galeri)", "OtoDNA kalkanlı araçlarınızı siber ağa mühürleyin.", Icons.directions_car_outlined, siberGold, () {
                Navigator.pop(ctx);
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("SİBER BİLGİ: Araç İlan Ekranı Açılıyor...")));
              }),
              SizedBox(height: 32),
            ],
          ),
  ...
  Widget _buildIlanSecenekKarti(String baslik, String altBaslik, IconData ikon, Color renk, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Container(padding: EdgeInsets.all(16), decoration: BoxDecoration(color: renk.withOpacity(0.1), shape: BoxShape.circle), child: Icon(ikon, color: renk, size: 24)),
            SizedBox(width: 20),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(baslik, style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold, fontFamily: 'Avenir')), SizedBox(height: 8), Text(altBaslik, style: TextStyle(color: Colors.white54, fontSize: 11, height: 1.4, fontFamily: 'Avenir'))])),
