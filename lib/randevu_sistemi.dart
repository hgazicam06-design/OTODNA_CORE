// lib/screens/randevu_sistemi.dart
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'dart:developer' as developer;

// SİBER NOT: Hizmet kütüphanesini import etmelisin (yolunu projene göre ayarla)
...
class _SiberRandevuSistemiState extends State<SiberRandevuSistemi> {
  ...
  // SİBER NOT: Gerçek projede bunu SiberHizmetKutuphanesi.tumHizmetleriGetir() ile çekmelisin.
  // Şimdilik test için kısa bir liste koyuyorum.
  final List<String> _hizmetListesi = [
    "Mekanik Bakım & Motor",
    "Şase & Alt Takım Kontrolü",
    "Elektrik & Beyin (ECU)",
    "Fırın Boya & Kaporta",
    "Ekspertiz & Check-Up",
    "EV Batarya Servisi"
  ...
  Future<void> _randevuyuMuhurle() async {
    if (_secilenHizmet == null || _secilenTarih == null || _secilenSaat == null) {
      ...
      _siberUyariGoster("SİBER İHLAL", "Hizmet, tarih ve saat seçimi zorunludur!", Colors.redAccent);
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        _siberUyariGoster("MÜHÜRLENDİ", "Randevunuz oluşturuldu. Bayi onayından sonra bildirim alacaksınız.", _kuantumCyan);
        Navigator.pop(context); // İşlem bitince ekranı kapat
      }
    } catch (e) {
      HapticFeedback.heavyImpact();
      developer.log("🚨 AĞ ÇÖKTÜ: Randevu iletilemedi!", error: e);
      _siberUyariGoster("BAĞLANTI HATASI", "Randevu oluşturulamadı. Matrix'i kontrol edin.", Colors.redAccent);
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("SİBER PLANLAMA VE RANDEVU", style: TextStyle(color: _kuantumCyan, fontWeight: FontWeight.w900, letterSpacing: 1.5, fontSize: 13)),
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: const Row(
                  children: [
                    Icon(Icons.calendar_month_outlined, color: _kuantumCyan, size: 28),
                    SizedBox(width: 12),
                    Expanded(child: Text("Siber ağ üzerinden randevunuzu mühürleyin. Ustanız sizin için hazırlık yapacaktır.", style: TextStyle(color: Colors.white70, fontSize: 11, height: 1.5, letterSpacing: 0.5))),
                  ],
                ),
              ),
              ...
              // 1. HİZMET SEÇİCİ
              const Text("1. İŞLEM TÜRÜ", style: TextStyle(color: Colors.white54, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.bold)),
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    isExpanded: true,
                    dropdownColor: _matGrey,
                    icon: const Icon(Icons.keyboard_arrow_down, color: _kuantumCyan),
                    hint: const Text("Hizmet Branşını Seçin", style: TextStyle(color: Colors.white30, fontSize: 13)),
              ...
              const Text("2. ZAMAN KOORDİNATLARI", style: TextStyle(color: Colors.white54, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Row(
                children: [
                  Expanded(
              ...
              // 3. ARIZA NOTU
              const Text("3. İSTİHBARAT NOTU (OPSİYONEL)", style: TextStyle(color: Colors.white54, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Container(
                decoration: BoxDecoration(
                  color: _matGrey,
              ...
              SizedBox(
                ...
                child: _islemSuruyor
                    ...
                    : ElevatedButton.icon(
                  icon: const Icon(Icons.verified, color: Colors.black, size: 24),
                  label: const Text("RANDEVUYU MÜHÜRLE", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.5, fontSize: 13)),
  ...
  Widget _buildZamanButonu({required String etiket, required IconData ikon, required VoidCallback onTap, required bool aktif}) {
    return OutlinedButton.icon(
      onPressed: onTap,
      icon: Icon(ikon, size: 18, color: aktif ? _kuantumCyan : Colors.white54),
      label: Text(etiket, style: TextStyle(color: aktif ? Colors.white : Colors.white54, fontWeight: aktif ? FontWeight.bold : FontWeight.normal, fontSize: 12)),
      style: OutlinedButton.styleFrom(
        padding: const EdgeInsets.symmetric(vertical: 16),
        side: BorderSide(color: aktif ? _kuantumCyan : Colors.white24, width: aktif ? 1.5 : 1),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        backgroundColor: aktif ? _kuantumCyan.withOpacity(0.05) : _matGrey,
      ),
    );
  }
}