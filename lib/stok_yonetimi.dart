// lib/screens/stok_yonetimi.dart
...
class _SiberStokPaneliState extends State<SiberStokPaneli> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("ENVANTER VE STOK RADARI", style: TextStyle(color: _kuantumCyan, fontWeight: FontWeight.w900, letterSpacing: 1.5, fontSize: 13)),
        backgroundColor: Colors.transparent,
        elevation: 0,
        iconTheme: const IconThemeData(color: _kuantumCyan),
      ),
      body: StreamBuilder<QuerySnapshot>(
        // 📡 SİBER NOT: Bayinin stokları canlı olarak dinleniyor!
          ...
          return Column(
            children: [
              ...
              if (stokListesi.isEmpty)
                const Expanded(
                  child: Center(
                    child: Text("SİBER ONAY: Envanterde ürün bulunmuyor.", style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold)),
                  ),
                )
              else
                Expanded(
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text("SİBER BARKOD OKU VE DÜŞ", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.5, fontSize: 13)),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildOzetKart("KRİTİK ALARM", "$kritik", _kanKirmizi),
          _buildOzetKart("ENVANTER DEĞERİ", "₺${deger.toStringAsFixed(0)}", _kuantumCyan),
        ],
      ),
    );
  }
  ...
  Widget _buildOzetKart(String baslik, String deger, Color renk) {
    return Column(
      children: [
        Text(baslik, style: const TextStyle(fontSize: 9, color: Colors.white54, fontWeight: FontWeight.bold, letterSpacing: 1)),
  ...
  Widget _buildSiberUrunKarti(Map<String, dynamic> urun) {
    ...
    Color kartRengi = kritikDurum ? _kanKirmizi : _kuantumCyan;

    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      decoration: BoxDecoration(
        color: kritikDurum ? _kanKirmizi.withOpacity(0.05) : _matGrey,
      ...
      child: ListTile(
        ...
        title: Text(urun['urun_adi'] ?? "Bilinmeyen Ürün", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13, letterSpacing: 0.5)),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const SizedBox(height: 6),
            Text("Stokta: $adet Adet (Sınır: $kritikSinir)", style: TextStyle(color: kritikDurum ? _kanKirmizi : Colors.white70, fontSize: 11, fontWeight: kritikDurum ? FontWeight.bold : FontWeight.normal)),
          ],
        ),
        trailing: Column(
          ...
          children: [
            Text("₺${fiyat.toStringAsFixed(2)}", style: const TextStyle(color: _kuantumCyan, fontWeight: FontWeight.w900, fontSize: 14)),
            if (kritikDurum) ...[
              const SizedBox(height: 4),
              const Text("STOK ALARMI!", style: TextStyle(color: _kanKirmizi, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1)),
            ]
          ],
        ),
      ),
...
class _SiberStokTarayiciEkraniState extends State<_SiberStokTarayiciEkrani> {
  ...
  Future<void> _stoktanDus(String okunanBarkod) async {
    ...
    try {
      ...
      if (urunSorgusu.docs.isEmpty) {
        throw Exception("Bu barkoda ait ürün envanterde bulunamadı!");
      }
      ...
      // 2. Ürünün adetini 1 eksilt (Otonom düşüm)
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Ürün stoktan düşüldü."), backgroundColor: SiberStokPaneli._kuantumCyan));
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString().replaceAll("Exception:", "").trim()), backgroundColor: SiberStokPaneli._kanKirmizi));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: SiberStokPaneli._oledBlack,
      appBar: AppBar(
        title: const Text("SİBER SATIŞ RADARI", style: TextStyle(color: SiberStokPaneli._kuantumCyan, fontWeight: FontWeight.w900)),
        backgroundColor: Colors.transparent,
        iconTheme: const IconThemeData(color: SiberStokPaneli._kuantumCyan),
      ),
      body: Stack(
        children: [
          MobileScanner(
          ...
          if (_islemSuruyor)
            Container(
              color: SiberStokPaneli._oledBlack.withOpacity(0.8),
              child: const Center(
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    CircularProgressIndicator(color: SiberStokPaneli._kuantumCyan),
                    SizedBox(height: 16),
                    Text("STOKTAN DÜŞÜLÜYOR...", style: TextStyle(color: SiberStokPaneli._kuantumCyan, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            )
