...
class SiberSuistimalKontrol {
  ...
  // ── 📡 GPS VE KULLANIM ŞARTLARI ANALİZİ (İSTİHBARAT RADARI) ──
  ...
  static Future<String> siberYolSartiAnalizi(String saseNo) async {
    developer.log("📡 SİBER İSTİHBARAT: $saseNo şaseli aracın GPS logları Karargahta analiz ediliyor...");

    try {
      // 1. Aracın son 30 günlük GPS loglarını Kuantum Ağından çek
      ...
      for (var doc in loglar.docs) {
        ...
        // 🧠 OTONOM SİBER ANALİZ: Kullanıcının limitleri aşıp aşmadığı taranıyor.
        if (data['arazi_vitesi_aktif'] == true || (data['hiz_limiti_asimi'] ?? 0) > 3 || data['zorlu_arazi_kodu'] == true) {
          zorluAraziIhlali++;
        }
      }

      // 🛡️ BAYİ VE USTA KORUMA KALKANI TETİKLENİYOR
      if (zorluAraziIhlali > 2) {
        developer.log("🚨 SUİSTİMAL TESPİTİ: Araç zorlu koşullarda limitleri zorlamış! Usta korunuyor.");
  ...
  // ── 📜 ÜRÜN BAZLI GARANTİ SORGULAMASI (KARARGAH MÜHRÜ) ──
  ...
  static Future<Map<String, dynamic>> garantiKosuluGetir(String urunBarkodu) async {
    ...
    try {
      ...
      // 🛑 Sarf malzeme ve garantisiz ürün kalkanı
      if (!garantiliMi) {
        return {
          "durum": "GARANTISIZ",
