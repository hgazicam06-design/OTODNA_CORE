...
class _EvBataryaMuhurTerminaliState extends State<EvBataryaMuhurTerminali> {
  ...
  Future<void> _evDnasiniMuhurle() async {
    if (_plakaController.text.isEmpty || _bataryaSagligiController.text.isEmpty || _tahminiMenzilController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Plaka, Batarya Sağlığı ve Menzil zorunludur! ⚡"), backgroundColor: Colors.redAccent));
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("⚡ EV Batarya DNA'sı Siber Ağa Mühürlendi!", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), backgroundColor: Color(0xFF00FFC2)));
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text("Hedef Araç Kimliği", style: TextStyle(color: SiberTema.kuantumCyan, fontSize: 14, fontWeight: FontWeight.bold)),
            SizedBox(height: 12),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                    ...
                    style: TextStyle(color: SiberTema.oledBlack, fontWeight: FontWeight.bold, fontSize: 16, letterSpacing: 1)
                ),
              ),
            ),
          ],
