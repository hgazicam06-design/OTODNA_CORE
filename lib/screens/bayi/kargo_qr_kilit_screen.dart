...
class _KargoQrKilitScreenState extends State<KargoQrKilitScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _qrPulseController = AnimationController(vsync: this, duration: Duration(seconds: 2))..repeat(reverse: true);
  }
      ...
      builder: (context) => StatefulBuilder(
          builder: (context, setModalState) {
            return Container(
              ...
              decoration: BoxDecoration(color: SiberTema.matGrey, borderRadius: BorderRadius.vertical(top: Radius.circular(24)), border: Border(top: BorderSide(color: SiberTema.kuantumCyan, width: 2))),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text("Kilitli Kargo QR Kodu Üretildi!", style: TextStyle(color: SiberTema.textMain, fontSize: 18, fontWeight: FontWeight.bold)),
                  SizedBox(height: 8),
                  Text("Sipariş: $urunAdi\nSipariş No: $siparisId", textAlign: TextAlign.center, style: TextStyle(color: SiberTema.kuantumCyan, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
                  SizedBox(height: 24),
                  ...
                  AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        return Container(
                          ...
                          child: Icon(Icons.qr_code_2, color: Colors.white, size: 150),
                        );
                      }
                  ),

                  SizedBox(height: 24),
                  Container(padding: EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.orangeAccent.withOpacity(0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.orangeAccent)), child: Row(children: [Icon(Icons.warning_amber_rounded, color: Colors.orangeAccent), SizedBox(width: 12), Expanded(child: Text("Bu QR Kodu paketin üzerine yapıştırın. Alıcı kargoyu teslim alırken kendi uygulamasından bu QR'ı okutmak zorundadır.", style: TextStyle(color: Colors.orangeAccent, fontSize: 11)))])),
                  SizedBox(height: 24),
                    ...
                    children: [
                      Expanded(
                          child: OutlinedButton.icon(
                              style: OutlinedButton.styleFrom(side: BorderSide(color: SiberTema.textMuted), padding: EdgeInsets.symmetric(vertical: 16)),
                              onPressed: isProcessing ? null : () => Navigator.pop(context),
                              icon: Icon(Icons.close, color: Colors.white),
                              label: Text("Kapat", style: TextStyle(color: SiberTema.textMain))
                          )
                      ),
                      SizedBox(width: 12),
                      Expanded(
                          child: ElevatedButton.icon(
                              ...
                              icon: isProcessing ? SizedBox() : Icon(Icons.print, color: Color(0xFF0F172A)),
                              label: isProcessing
                                  ? SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
                                  : Text("Yazdır & Gönder", style: TextStyle(color: Color(0xFF0F172A), fontWeight: FontWeight.bold))
                          )
                      ),
                    ],
                  )
  ...
  String _kalanZamaniHesapla(Timestamp? islemTarihi) {
    ...
    DateTime bitisZamani = islemZamani.add(Duration(days: 15));
  ...
  Widget build(BuildContext context) {
    ...
    if (_currentUser == null) {
      return Scaffold(backgroundColor: bgColor, body: Center(child: Text("Siber Kimlik Bulunamadı!", style: TextStyle(color: SiberTema.kanKirmizi))));
    }
    ...
    return ResponsiveKalkan(
      ...
      child: Scaffold(
        ...
        title: Text('Kargo & Güvenli Havuz', style: TextStyle(color: SiberTema.textMain, fontWeight: FontWeight.bold)),
        ...
        bottom: TabBar(
          ...
          tabs: [Tab(text: "Bekleyen Kargolar"), Tab(text: "15 Günlük Havuz")],
        ),
      ),
        ...
        // Ustanın Tereddütsüz Tüm Siparişlerini Çek (Dart tarafında filtreleyerek Index hatasını önleriz)
          ...
          builder: (context, snapshot) {
            ...
            if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
              return Center(child: Text("OtoDNA Ağında size ait bir sipariş bulunmuyor.", style: TextStyle(color: SiberTema.textMuted)));
            }
            ...
            return TabBarView(
              ...
              children: [
                ...
                // =================================================================
                bekleyenler.isEmpty
                    ? Center(child: Text("Bekleyen kargonuz bulunmuyor. ✅", style: TextStyle(color: primaryCyan)))
                    : ListView.builder(
                  padding: EdgeInsets.all(16), physics: BouncingScrollPhysics(), itemCount: bekleyenler.length,
                  itemBuilder: (context, index) {
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(doc.id.substring(0, 10).toUpperCase(), style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontWeight: FontWeight.bold)), Text(formatliTarih, style: TextStyle(color: SiberTema.textMuted, fontSize: 11))]),
                          SizedBox(height: 12),
                          Text(siparis['urun_adi'] ?? 'Bilinmeyen Ürün', style: TextStyle(color: SiberTema.textMain, fontSize: 16, fontWeight: FontWeight.bold)),
                          SizedBox(height: 4),
                          Row(children: [Icon(Icons.person, color: primaryCyan, size: 14), SizedBox(width: 4), Text("Alıcı: ${siparis['alici_adi'] ?? '-'}", style: TextStyle(color: primaryCyan, fontSize: 13))]),
                          SizedBox(height: 12),
                          ...
                          Row(
                            ...
                            children: [
                              Text("₺${siparis['fiyat'] ?? 0}", style: TextStyle(color: SiberTema.textMain, fontSize: 18, fontWeight: FontWeight.bold)),
                              ElevatedButton.icon(
                                  ...
                                  icon: Icon(Icons.qr_code, color: bgColor, size: 16),
                                  label: Text("Kargo QR Üret", style: TextStyle(color: bgColor, fontWeight: FontWeight.bold))
                              ),
                            ],
                          )
                        ],
                ...
                havuzdakiler.isEmpty
                    ? Center(child: Text("Havuzda bekleyen bakiye/sipariş yok.", style: TextStyle(color: SiberTema.textMuted)))
                    : ListView.builder(
                  padding: EdgeInsets.all(16), physics: BouncingScrollPhysics(), itemCount: havuzdakiler.length,
                  itemBuilder: (context, index) {
                    ...
                    return Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(doc.id.substring(0, 10).toUpperCase(), style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontWeight: FontWeight.bold)), Icon(Icons.security, color: primaryCyan, size: 16)]),
                          SizedBox(height: 12),
                          Text(siparis['urun_adi'] ?? 'Bilinmeyen Ürün', style: TextStyle(color: SiberTema.textMain, fontSize: 16, fontWeight: FontWeight.bold)),
                          SizedBox(height: 8),
                          Container(padding: EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: durumRengi.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Text(durum, style: TextStyle(color: durumRengi, fontSize: 11, fontWeight: FontWeight.bold))),
                          SizedBox(height: 16),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Kalan Havuz Süresi", style: TextStyle(color: SiberTema.textMuted, fontSize: 10)), SizedBox(height: 4), Text(kalanZaman, style: TextStyle(color: durumRengi, fontSize: 14, fontWeight: FontWeight.bold))]),
                              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text("Aktarılacak Tutar", style: TextStyle(color: SiberTema.textMuted, fontSize: 10)), SizedBox(height: 4), Text("₺${siparis['fiyat'] ?? 0}", style: TextStyle(color: SiberTema.textMain, fontSize: 18, fontWeight: FontWeight.bold))]),
                            ],
                          )
                        ],
                      ),
