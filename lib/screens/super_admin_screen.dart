...
class _SuperAdminScreenState extends State<SuperAdminScreen> with TickerProviderStateMixin {
  ...
  static Color adminPurple = Colors.purpleAccent;
  ...
  void _siberMesajGoster(String mesaj, {bool isError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(mesaj, style: TextStyle(fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 1, fontFamily: 'Avenir')),
    ...
    return ResponsiveKalkan(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              icon: Icon(Icons.settings_suggest, color: SiberTema.siberGold),
              tooltip: 'Komisyon Ayarları',
              onPressed: () {
                HapticFeedback.lightImpact();
                context.push('/komisyon_ayarlari');
              },
            ),
            IconButton(
              icon: Icon(Icons.list_alt, color: SiberTema.kuantumCyan),
              tooltip: 'Siber Loglar',
              onPressed: () {
                HapticFeedback.lightImpact();
                context.push('/siber_istihbarat_loglari');
              },
            ),
          ],
          bottom: TabBar(
            ...
            isScrollable: true,
            labelStyle: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1, fontFamily: 'Avenir'),
            tabs: [
        ...
        return ListView(
          ...
          children: [
            Container(
              ...
              child: Column(
                children: [
                  Text("OTODNA KÜRESEL SİSTEM HACMİ", style: TextStyle(color: SiberTema.textMuted, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 2, fontFamily: 'Avenir')),
                  SizedBox(height: 12),
                  Text("₺ ${data['toplam_hacim']}", style: TextStyle(color: SiberTema.textMain, fontSize: 40, fontWeight: FontWeight.w900, letterSpacing: 2, fontFamily: 'Avenir')),
                ],
              ),
            ),
            SizedBox(height: 24),
            ...
            SizedBox(height: 48),
            Text("SON OTONOM İŞLEMLER", style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 2, fontFamily: 'Avenir')),
            SizedBox(height: 16),
  ...
  Widget _buildMuhurSekmesi() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) return Center(child: CircularProgressIndicator(color: adminPurple));
          ...
          itemBuilder: (context, index) {
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    Container(padding: EdgeInsets.all(16), decoration: BoxDecoration(color: adminPurple.withOpacity(0.1), shape: BoxShape.circle), child: Icon(Icons.lock_clock, color: adminPurple)),
                    SizedBox(width: 20),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text("İPTAL TALEBİ: $id", style: TextStyle(color: SiberTema.textMain, fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1, fontFamily: 'Avenir')),
                          SizedBox(height: 6),
                          Text("İkili Onay Sağlandı: Müşteri & Firma", style: TextStyle(color: adminPurple, fontSize: 10, fontWeight: FontWeight.bold, fontFamily: 'Avenir'))
                        ],
                      ),
                    ),
                    Icon(Icons.arrow_forward_ios, color: adminPurple, size: 16)
                  ],
                ),
              ),
            );
  ...
  Widget _buildSosSekmesi() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData) return Center(child: CircularProgressIndicator(color: SiberTema.kanKirmizi));
        return ListView(
          padding: EdgeInsets.all(24),
          children: [
            _buildRadarVizuel(),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text("TÜM BAYİLERİN ROZET VE YILDIZ YÖNETİMİ", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.5, fontFamily: 'Avenir')),
                onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => SiberBayiRozetYonetimiScreen())),
              ),
            ),
            SizedBox(height: 12),
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text('BLACK STAR KARALİSTE YÖNETİMİ', style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.5, fontFamily: 'Avenir')),
                onPressed: () => context.push('/karaliste_yonetimi'),
              ),
            ),
            SizedBox(height: 40),

            Text("SİBER İHLAL YAPAN FİRMALAR (BLACKLIST)", style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 2, fontFamily: 'Avenir')),
            SizedBox(height: 16),
            ...snapshot.data!.docs.map((doc) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 16),
          Text(tutar, style: TextStyle(color: SiberTema.textMain, fontSize: 18, fontWeight: FontWeight.w900, letterSpacing: 1, fontFamily: 'Avenir')),
          SizedBox(height: 8),
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(islem, style: TextStyle(color: SiberTema.textMain, fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 1.5, fontFamily: 'Avenir')),
                SizedBox(height: 4),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(plaka, style: TextStyle(color: SiberTema.textMain, fontSize: 12, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Expanded(child: Text(firma, style: TextStyle(color: SiberTema.kanKirmizi, fontWeight: FontWeight.w900, fontSize: 12, fontFamily: 'Avenir'))),
          ...
          Divider(color: SiberTema.textMuted, height: 24),
          Text("SİBER İHLAL: $sebep", style: TextStyle(color: SiberTema.textMuted, fontSize: 10, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
          SizedBox(height: 16),
          Row(
            mainAxisAlignment: MainAxisAlignment.end,
            children: [
              TextButton(onPressed: () {}, child: Text("UYARI SİNYALİ", style: TextStyle(color: Colors.orangeAccent, fontSize: 10, fontWeight: FontWeight.w900, fontFamily: 'Avenir'))),
              SizedBox(width: 12),
              ElevatedButton(
                ...
                onPressed: () {},
                child: Text("MEN ET", style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
              ),
            ],
          )
        ],
  ...
  Widget _buildBosEkran(String mesaj) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.shield_outlined, color: SiberTema.textMuted, size: 64),
          SizedBox(height: 16),
          Text(mesaj, style: TextStyle(color: SiberTema.textMuted, fontWeight: FontWeight.w900, letterSpacing: 2, fontFamily: 'Avenir')),
        ],
      ),
    );
  }
  ...
  void _muhurKirmaTalebiIncele(String islemId, Map<String, dynamic> itiraz) {
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        padding: EdgeInsets.all(24),
        decoration: BoxDecoration(color: surfaceColor, borderRadius: BorderRadius.vertical(top: Radius.circular(24)), border: Border(top: BorderSide(color: adminPurple, width: 2))),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("ADMİN YETKİSİ: MÜHÜR KIRMA", style: TextStyle(color: SiberTema.textMain, fontSize: 13, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
            SizedBox(height: 24),
            Text("İTİRAZ SEBEBİ: ${itiraz['sebep']}", style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontFamily: 'Avenir')),
            SizedBox(height: 32),
            SizedBox(
              width: double.infinity,
              height: 56,
              child: ElevatedButton(
                ...
                child: Text("MÜHRÜ KIR VE ONAYLA", style: TextStyle(fontWeight: FontWeight.w900, color: Colors.white, fontFamily: 'Avenir')),
              ),
            )
          ],
        ),
