...
class DistributorMerkezUssu extends StatefulWidget {
  DistributorMerkezUssu({super.key});
...
class _DistributorMerkezUssuState extends State<DistributorMerkezUssu> {
  ...
  Widget build(BuildContext context) {
    if (_currentUser == null) {
      return Scaffold(backgroundColor: Colors.black, body: Center(child: Text("Siber Kimlik Bulunamadı!", style: TextStyle(color: Colors.red))));
    }
    ...
    return ResponsiveKalkan(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(icon: Icon(Icons.arrow_back_ios_new, color: SiberTema.altinSari, size: 18), onPressed: () => Navigator.pop(context)),
          title: Text("KÜRESEL DİSTRİBÜTÖR AĞI", style: TextStyle(color: SiberTema.altinSari, fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 3, fontFamily: 'Avenir')),
          centerTitle: true,
          actions: [IconButton(icon: Icon(Icons.radar, color: SiberTema.kuantumCyan, size: 20), onPressed: () {})],
        ),
        body: StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              if (!snapshot.hasData || !snapshot.data!.exists) return Center(child: Text("B2B Verisi Yükleniyor...", style: TextStyle(color: SiberTema.textMuted)));
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: Color(0xFF1A1A1D), // Koyu Titanyum
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.precision_manufacturing_outlined, color: SiberTema.altinSari, size: 40),
                          SizedBox(width: 20),
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(firmaAdi.toUpperCase(), style: TextStyle(color: SiberTema.textMain, fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: -0.5, fontFamily: 'Avenir')),
                                SizedBox(height: 6),
                                Row(
                                  children: [
                    ...
                    Text("B2B Kitle Yükleme Merkezleri", style: TextStyle(color: SiberTema.textMuted, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
                    SizedBox(height: 16),
                    GridView.count(
                      crossAxisCount: 2,
                      crossAxisSpacing: 16,
                    ...
                    Text("Küresel Borsa & Ciro Akışı", style: TextStyle(color: SiberTema.textMuted, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
                    SizedBox(height: 16),
                    Container(
                      padding: EdgeInsets.all(24),
                      decoration: BoxDecoration(color: Color(0xFF1A1A1D), borderRadius: BorderRadius.circular(20), border: Border.all(color: SiberTema.textMuted)),
                      child: Column(
                        children: [
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Toplam Toptan Hacim", style: TextStyle(color: SiberTema.textMuted, fontSize: 13)), Text("₺${(aylikCiro/1000000).toStringAsFixed(2)} Milyon", style: TextStyle(color: SiberTema.textMain, fontSize: 16, fontWeight: FontWeight.bold))]),
                          Padding(padding: EdgeInsets.symmetric(vertical: 16), child: Divider(color: SiberTema.textMuted, height: 1)),
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Karargah Payı (VIP %10)", style: TextStyle(color: SiberTema.kanKirmizi, fontSize: 13)), Text("-₺${otodnaKesintisi.toStringAsFixed(2)}", style: TextStyle(color: SiberTema.kanKirmizi, fontSize: 16, fontWeight: FontWeight.bold))]),
                          Padding(padding: EdgeInsets.symmetric(vertical: 16), child: Divider(color: SiberTema.textMuted, height: 1)),
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Net Aktarım", style: TextStyle(color: SiberTema.altinSari, fontSize: 14, fontWeight: FontWeight.bold)), Text("₺${netHakedis.toStringAsFixed(2)}", style: TextStyle(color: SiberTema.altinSari, fontSize: 22, fontWeight: FontWeight.w900))]),
                        ],
                      ),
                    ),
                    SizedBox(height: 32),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text(baslik, textAlign: TextAlign.center, style: TextStyle(color: SiberTema.textMain, fontSize: 13, fontWeight: FontWeight.bold, fontFamily: 'Avenir', height: 1.3)),
          ],
        ),
      ),
    );
  ...
  Widget _buildSiberGeniIslemButonu(String baslik, String altBaslik, IconData ikon, Color renk, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Color(0xFF1A1A1D), borderRadius: BorderRadius.circular(20), border: Border.all(color: renk.withOpacity(0.3))),
        child: Row(
          children: [
  ...
  void _siberUyari(String mesaj) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(mesaj, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontFamily: 'Avenir')), backgroundColor: SiberTema.altinSari));
  }
}
