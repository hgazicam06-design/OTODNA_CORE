...
class _UstaPaneliScreenState extends State<UstaPaneliScreen> {
  ...
  void _rehberiGoster({bool otomatik = false}) {
    String baslik = "OTODNA USTA PANELİ";
    String icerik = "Siber Karargah Usta Ağına Hoş Geldiniz.\n\n"
  ...
  void _sasedenSorgula() async {
    if (_saseController.text.length < 17) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Şase Numarası 17 Haneli Olmalıdır!'), backgroundColor: Colors.redAccent));
      return;
    }
    ...
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Araç Fabrika Verileri Hub\'dan Çekildi! ✅'), backgroundColor: Colors.green));
  }

  void _musteriOnayinaGonder() async {
    if (_testSonuclari.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Hiçbir testi tamamlamadınız!'), backgroundColor: Colors.orangeAccent));
      return;
    }
    
    if (_saseController.text.length < 17) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Mühürleme için 17 Haneli Şase numarası girmelisiniz!'), backgroundColor: Colors.redAccent));
      return;
    }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: BorderSide(color: Color(0xFF00FFC2), width: 1.5)),
        content: Column(
    ...
    if (sonuc['basarili']) {
      showDialog(
        ...
        barrierDismissible: false,
        builder: (context) => AlertDialog(
          backgroundColor: Color(0xFF121B2B), // Dijital Kale Kart Rengi
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: BorderSide(color: Colors.orangeAccent, width: 1.5)),
          title: Column(
            children: [
          ...
          actions: [
            TextButton(
              ...
              child: Text("YENİ ARAÇ İNCELE", style: TextStyle(color: Colors.orangeAccent, fontWeight: FontWeight.bold)),
            )
          ],
        ),
      );
  ...
  List<Widget> _dinamikModulleriGetir() {
    ...
    if (_secilenAracCinsi == null || _secilenYakitTipi == null) {
      return [Center(child: Padding(padding: EdgeInsets.all(20.0), child: Text("Çalışır aksamların listelenmesi için Araç Cinsi ve Yakıt Tipi seçiniz.", textAlign: TextAlign.center, style: TextStyle(color: SiberTema.textMuted))))];
    }
    ...
    moduller.add(SizedBox(height: 24));
    moduller.add(
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            icon: Icon(Icons.send_to_mobile, size: 28),
            label: Text("MÜŞTERİ ONAYINA GÖNDER", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900, letterSpacing: 1)),
          ),
        )
    );
  ...
  Widget _buildKayitMotoruKarti(String baslik, IconData ikon, {bool isCritical = false, bool zorunluFoto = false}) {
    ...
    Color kartRengi = Color(0xFF121B2B); // Dijital Kale Kart Rengi
    Color cerceveRengi = isCritical ? Colors.redAccent.withOpacity(0.5) : Colors.white12;
    Color ikonRengi = isCritical ? Colors.redAccent : Color(0xFF00FFC2);
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (zorunluFoto) ...[SizedBox(height: 4), Text("Zorunlu Görsel Kanıt", style: TextStyle(color: Colors.orangeAccent, fontSize: 10, fontWeight: FontWeight.bold))],
                if (durum != 'bekliyor') ...[
                  SizedBox(height: 4),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            color: secili ? Color(0xFF00FFC2).withOpacity(0.15) : Color(0xFF121B2B),
            borderRadius: BorderRadius.circular(20),
            border: Border.all(color: secili ? Color(0xFF00FFC2) : Colors.white12)
        ),
        child: Text(etiket, style: TextStyle(color: secili ? Color(0xFF00FFC2) : Colors.white54, fontSize: 12, fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          elevation: 0,
          shape: Border(bottom: BorderSide(color: SiberTema.textMuted, width: 1)),
          title: Text('OtoDNA Usta Paneli', style: TextStyle(color: SiberTema.textMain, fontWeight: FontWeight.bold, fontSize: 16, letterSpacing: 1)),
          iconTheme: IconThemeData(color: primaryCyan),
          centerTitle: true,
          actions: [
            IconButton(
              icon: Icon(Icons.help_outline_rounded, color: primaryCyan),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => SiberServisKabulPaneli()));
                },
                child: Container(
                  width: double.infinity,
                  padding: EdgeInsets.all(16),
                  decoration: BoxDecoration(
                    color: Color(0xFF00FFC2).withOpacity(0.1),
                    borderRadius: BorderRadius.circular(16),
                    border: Border.all(color: Color(0xFF00FFC2).withOpacity(0.5)),
                  ),
                  child: Row(
                    children: [
              ...
              Text("SİBER HUB SORGULAMA", style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
              SizedBox(height: 12),
              Row(
                children: [
                  Expanded(child: TextField(controller: _saseController, style: TextStyle(color: SiberTema.textMain, letterSpacing: 2, fontWeight: FontWeight.bold), textCapitalization: TextCapitalization.characters, maxLength: 17, decoration: InputDecoration(hintText: '17 Haneli Şase (VIN)', hintStyle: TextStyle(color: SiberTema.textMuted), filled: true, fillColor: cardColor, counterText: "", border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: SiberTema.textMuted))))),
                  SizedBox(width: 12),
                  GestureDetector(
                    onTap: _veriCekiliyor ? null : _sasedenSorgula,
                    child: Container(padding: EdgeInsets.symmetric(horizontal: 20, vertical: 16), decoration: BoxDecoration(color: primaryCyan, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: primaryCyan.withOpacity(0.3), blurRadius: 10)]), child: _veriCekiliyor ? SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: bgColor, strokeWidth: 2)) : Icon(Icons.search, color: bgColor)),
                  )
                ],
              ),
              SizedBox(height: 24),
              ...
              Text("HIZLI ARAÇ SINIFI FİLTRESİ", style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
              SizedBox(height: 12),
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    dropdownColor: cardColor, isExpanded: true, value: _secilenAracCinsi, hint: Text("Tüm Araç Cinsleri Listesi...", style: TextStyle(color: SiberTema.textMuted, fontSize: 13)),
                    icon: Icon(Icons.keyboard_arrow_down, color: primaryCyan),
                    items: _aracCinsleri.map((String cins) { return DropdownMenuItem<String>(value: cins, child: Text(cins, style: TextStyle(color: SiberTema.textMain, fontSize: 13))); }).toList(),
              ...
              Text("MOTOR VE YAKIT TİPİ", style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
              SizedBox(height: 12),
              Container(
                padding: EdgeInsets.symmetric(horizontal: 16),
                decoration: BoxDecoration(color: cardColor, borderRadius: BorderRadius.circular(16), border: Border.all(color: SiberTema.textMuted)),
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    dropdownColor: cardColor, isExpanded: true, value: _secilenYakitTipi, hint: Text("Yakıt / Motor Tipi Seçiniz...", style: TextStyle(color: SiberTema.textMuted, fontSize: 13)),
                    icon: Icon(Icons.keyboard_arrow_down, color: primaryCyan),
                    items: _yakitTipleri.map((String yakit) { return DropdownMenuItem<String>(value: yakit, child: Text(yakit, style: TextStyle(color: SiberTema.textMain, fontSize: 13))); }).toList(),
