...
class _SosProtokoluScreenState extends State<SosProtokoluScreen> with SingleTickerProviderStateMixin {
  ...
  void _sinyalBaslat(TapDownDetails details) {
    ...
    _sinyalZamanlayici = Timer.periodic(Duration(milliseconds: 100), (timer) {
      setState(() {
  ...
  Future<void> _matrikseSosGonder() async {
    ...
    try {
      ...
      await _db.collection('sos_sinyalleri').add({
        ...
        'konum': GeoPoint(39.92077, 32.85411), // Ankara Merkez (Karargah)
  ...
  void _siberUyariVer(String mesaj) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(mesaj, style: TextStyle(fontWeight: FontWeight.w900, fontFamily: 'Avenir', letterSpacing: 1)),
              ...
              child: Column(
                ...
                children: [
                  if (!_sinyalGonderildi) ...[
                    ...
                    Icon(Icons.radar, color: SiberTema.kuantumCyan, size: 60),
                    SizedBox(height: 20),
                    Text("SİNYAL YAYINDA", style: TextStyle(color: SiberTema.textMain, fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: 4, fontFamily: 'Avenir')),
                    SizedBox(height: 8),
