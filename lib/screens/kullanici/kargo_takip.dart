...
class SiberKargoTakipEkrani extends StatelessWidget {
  ...
  void _bayiyeSiberHatAc(BuildContext context, String bayiId) {
    ...
    _siberUyariGoster(context, "BAYİ HATTI", "Bayi ile kriptolu bağlantı kuruluyor...", SiberTema.kuantumCyan);
  }

  void _karargahaBaglan(BuildContext context) {
    HapticFeedback.heavyImpact();
    developer.log("🚨 ACİL HAT: Doğrudan Merkez Karargaha (Gazi'ye) bağlantı talebi!");
    _siberUyariGoster(context, "MERKEZ KARARGAH", "Gazi Komutaya bağlanılıyor. Lütfen hatta kalın.", SiberTema.kanKirmizi);
  }
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Container(
            height: 100,
            decoration: BoxDecoration(color: SiberTema.matGrey.withOpacity(0.8), borderRadius: BorderRadius.circular(16)),
        ...
        if (!snapshot.hasData || !snapshot.data!.exists) {
          return Container(
            ...
            decoration: BoxDecoration(color: SiberTema.matGrey.withOpacity(0.8), borderRadius: BorderRadius.circular(16), border: Border.all(color: SiberTema.kanKirmizi.withOpacity(0.5))),
            child: const Text("SİBER İHLAL: Kargo verisi bulunamadı!", style: TextStyle(color: SiberTema.kanKirmizi, fontWeight: FontWeight.w900, letterSpacing: 1)),
          );
        }
        ...
        // Duruma göre otonom renk seçimi (Yoldaysa Turkuaz, Bekliyorsa Altın/Turuncu)
              ...
              Row(
                ...
                children: [
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text("LOJİSTİK DURUMU", style: TextStyle(color: Colors.white54, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.w900)),
                      const SizedBox(height: 4),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: SiberTema.kuantumCyan.withOpacity(0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: SiberTema.kuantumCyan.withOpacity(0.5))),
                    child: Row(
                      children: [
                        const Icon(Icons.qr_code_scanner, color: SiberTema.kuantumCyan, size: 16),
              ...
              Row(
                children: [
                  ...
                  _buildSiberIletisimButonu(
                      ...
                      renk: SiberTema.kanKirmizi, // Admin hattı her zaman kan kırmızısıdır!
