...
class _IlanVerFotografAciklamaScreenState extends State<IlanVerFotografAciklamaScreen> {
  ...
  Future<void> _galeridenFotoSec(int index) async {
    ...
    if (secilenGorsel != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text("Görsel ağa eklendi.", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontFamily: 'Avenir')), backgroundColor: Colors.teal.shade700, duration: const Duration(seconds: 1)));
    }
  }
  ...
  Future<void> _ilaniYayinaAl() async {
    ...
    if (_aciklamaController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Lütfen alıcılar için bir donanım açıklaması girin.", style: TextStyle(color: Colors.white, fontFamily: 'Avenir')), backgroundColor: Colors.redAccent));
      return;
    }
    if (_anaFoto == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("En azından Ana Kapak Fotoğrafını yüklemelisiniz.", style: TextStyle(color: Colors.white, fontFamily: 'Avenir')), backgroundColor: Colors.orangeAccent));
      return;
    }
    ...
    try {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text("${widget.secilenArac.join(' ')} aracınız Plaza Ağına başarıyla yüklendi ve yayına alındı.", style: const TextStyle(color: Colors.black54, fontSize: 13, height: 1.5, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                const SizedBox(height: 24),
                Container(
                  padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFFFAFAFC), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.black.withValues(alpha: 0.05))),
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text("SATIŞ FİYATI", style: TextStyle(color: Colors.black38, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1, fontFamily: 'Avenir')),
                      ...
                      const Text("HAVUZ GÜVENCESİ", style: TextStyle(color: Colors.black38, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1, fontFamily: 'Avenir')),
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Ağa bağlanırken hata oluştu: $e", style: const TextStyle(color: Colors.white, fontFamily: 'Avenir')), backgroundColor: Colors.redAccent));
    } finally {
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text("${widget.secilenArac.join(' > ')}\nAracınızın görsellerini ve son donanım detaylarını ağa yükleyin.", style: const TextStyle(color: Colors.black54, fontSize: 13, height: 1.5, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                    const SizedBox(height: 40),

                    // Fiyat Bilgisi
                    const Text("Belirlenen Satış Fiyatı", style: TextStyle(color: Colors.black45, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1, fontFamily: 'Avenir')),
                    const SizedBox(height: 8),
                    ...
                    Container(
                      ...
                      child: TextField(
                          ...
                          decoration: const InputDecoration(
                              ...
                              hintStyle: TextStyle(color: Colors.black26, fontSize: 13, fontFamily: 'Avenir'),
                    ...
                    Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              ...
                              else BoxShadow(color: Colors.black.withValues(alpha: 0.02), blurRadius: 10, offset: const Offset(0, 5))
                            ]
                        ),
                        child: Row(
                            children: [
                              ...
                              Expanded(
                                  child: Column(
                                      ...
                                      children: [
                                        ...
                                        const Text("İlanınızda 15 günlük 'Güvenli Havuz' rozeti çıkar, alıcı güveni artar.", style: TextStyle(color: Colors.black54, fontSize: 11, height: 1.4, fontWeight: FontWeight.bold, fontFamily: 'Avenir'))
                                      ]
                                  )
                              ),
                              Switch(
      ...
      child: AspectRatio(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
              ...
              boxShadow: seciliFoto != null ? [BoxShadow(color: primaryTeal.withValues(alpha: 0.1), blurRadius: 10)] : [BoxShadow(color: Colors.black.withValues(alpha: 0.02), blurRadius: 5)]
          ),
          child: seciliFoto != null
            ...
            child: Column(
              ...
              children: [
                Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.black.withValues(alpha: 0.02), shape: BoxShape.circle), child: const Icon(Icons.add_a_photo_outlined, color: Colors.black26, size: 24)),
                if (isMain) const SizedBox(height: 12),
                if (isMain) const Text("Ana Kapak", style: TextStyle(color: Colors.black45, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 0.5, fontFamily: 'Avenir'))
              ],
            ),
          ),
        ),
