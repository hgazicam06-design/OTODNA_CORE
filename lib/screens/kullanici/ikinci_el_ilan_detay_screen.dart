...
class IkinciElIlanDetayScreen extends StatefulWidget {
  ...
  IkinciElIlanDetayScreen({super.key, required this.ilan});
...
class _IkinciElIlanDetayScreenState extends State<IkinciElIlanDetayScreen> {
  ...
  Widget _buildSaticiRozeti(int puan, String saticiAdi) {
    ...
    if (puan == 5) { rozetRengi = Colors.amber.shade700; rozetMetni = "Altın Rozetli Bayi"; }
      ...
      return Row(children: [Icon(Icons.star_border, color: Colors.white38, size: 16), SizedBox(width: 8), Text(saticiAdi, style: TextStyle(color: textColor, fontSize: 14, fontFamily: 'Avenir', fontWeight: FontWeight.bold))]);
    } else {
      return Row(children: [Icon(Icons.warning_amber_rounded, color: dangerColor, size: 16), SizedBox(width: 8), Text("$saticiAdi (Kara Liste)", style: TextStyle(color: dangerColor, fontSize: 14, decoration: TextDecoration.lineThrough, fontFamily: 'Avenir', fontWeight: FontWeight.bold))]);
    }
    ...
    return Row(
      children: [
        ...
        SizedBox(width: 12),
        Container(padding: EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: rozetRengi.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: rozetRengi.withValues(alpha: 0.5))), child: Text(rozetMetni, style: TextStyle(color: rozetRengi, fontSize: 10, fontWeight: FontWeight.bold, fontFamily: 'Avenir'))),
      ],
    );
  }
  ...
  void _kaporaSistemiAc(String fiyatStr) {
    String kaporaBedeli = "10.000 TL";

    showModalBottomSheet(
      context: context, isScrollControlled: true, backgroundColor: Colors.transparent,
      builder: (context) => Container(
        padding: EdgeInsets.all(32),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.vertical(top: Radius.circular(32)), boxShadow: [BoxShadow(color: Colors.white.withValues(alpha: 0.1), blurRadius: 20)]),
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 32),
            Row(children: [Container(padding: EdgeInsets.all(10), decoration: BoxDecoration(color: primaryTeal.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(Icons.lock_clock_outlined, color: primaryTeal, size: 24)), SizedBox(width: 16), Text("Araç Rezervasyonu", style: TextStyle(color: textColor, fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: -0.5, fontFamily: 'Avenir'))]),
            SizedBox(height: 16),
            Text("Bu aracı sizin adınıza ayırtmak ve satıcının doğrudan iletişim bilgilerine erişmek için OtoDNA Havuzuna kapora yatırmanız gerekmektedir.", style: TextStyle(color: Colors.white54, fontSize: 13, height: 1.5, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
            SizedBox(height: 32),

            // ÜCRET KUTUSU
            Container(
              padding: EdgeInsets.all(20), decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white.withValues(alpha: 0.05))),
              child: Column(
                children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("İlan Satış Fiyatı", style: TextStyle(color: Colors.white54, fontSize: 14, fontWeight: FontWeight.bold, fontFamily: 'Avenir')), Text("₺$fiyatStr", style: TextStyle(color: Colors.white38, fontSize: 14, decoration: TextDecoration.lineThrough, fontWeight: FontWeight.bold, fontFamily: 'Avenir'))]),
                  Padding(padding: EdgeInsets.symmetric(vertical: 16.0), child: Divider(color: Colors.white.withValues(alpha: 0.05), height: 1)),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Kapora Ödendi! Satıcı İletişim Bilgileri Açıldı. ✅', style: TextStyle(fontFamily: 'Avenir', fontWeight: FontWeight.bold)), backgroundColor: primaryTeal));
                },
                child: Text("$kaporaBedeli YATIR VE REZERVE ET", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 1.5, fontFamily: 'Avenir')),
              ),
            ),
            SizedBox(height: 16),
          ],
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // TEKNİK DETAYLAR
                    GridView.count(
                      shrinkWrap: true, physics: NeverScrollableScrollPhysics(), crossAxisCount: 2, childAspectRatio: 2.5, crossAxisSpacing: 16, mainAxisSpacing: 16,
                      children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildAracParcasi("Ön Tampon", 140, 30, BorderRadius.vertical(top: Radius.circular(30))),
                          SizedBox(height: 4),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              _buildAracParcasi("Sol Ön Çamurluk", 35, 80, BorderRadius.only(topLeft: Radius.circular(20), bottomLeft: Radius.circular(5))),
                              SizedBox(width: 4),
                              _buildAracParcasi("Kaput", 100, 80, BorderRadius.circular(8)),
                              SizedBox(width: 4),
                              _buildAracParcasi("Sağ Ön Çamurluk", 35, 80, BorderRadius.only(topRight: Radius.circular(20), bottomRight: Radius.circular(5))),
                            ],
                          ),
                          SizedBox(height: 4),
                          Row(
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              SizedBox(width: 108), // Tavan genisliği + bosluklar
                          ...
                          Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              _buildAracParcasi("Sol Arka Çamurluk", 35, 80, BorderRadius.only(topLeft: Radius.circular(5), bottomLeft: Radius.circular(20))),
                              SizedBox(width: 4),
                              _buildAracParcasi("Bagaj", 100, 80, BorderRadius.circular(8)),
                              SizedBox(width: 4),
                              _buildAracParcasi("Sağ Arka Çamurluk", 35, 80, BorderRadius.only(topRight: Radius.circular(5), bottomRight: Radius.circular(20))),
                            ],
                          ),
                          SizedBox(height: 4),
                          _buildAracParcasi("Arka Tampon", 140, 30, BorderRadius.vertical(bottom: Radius.circular(30))),
                          ...
                          Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Icon(Icons.circle, color: primaryTeal.withValues(alpha: 0.2), size: 12), SizedBox(width: 6), Text("Orijinal", style: TextStyle(color: Colors.white54, fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Avenir')), SizedBox(width: 16),
                              Icon(Icons.circle, color: warningColor, size: 12), SizedBox(width: 6), Text("Boyalı", style: TextStyle(color: Colors.white54, fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Avenir')), SizedBox(width: 16),
                              Icon(Icons.circle, color: dangerColor, size: 12), SizedBox(width: 6), Text("Değişmiş", style: TextStyle(color: Colors.white54, fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                            ],
                          ),
                        ],
                      ),
                ...
                Container(
                  ...
                  child: IconButton(
                    ...
                    onPressed: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Satıcıya Mesaj Penceresi Açılıyor...', style: TextStyle(fontFamily: 'Avenir', fontWeight: FontWeight.bold)), backgroundColor: primaryTeal));
                    },
                  ),
                ),
                SizedBox(width: 16),
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        if (!havuzVarMi || karaListeMi) {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Bu ilan OtoDNA Güvencesine kapalıdır!', style: TextStyle(fontFamily: 'Avenir', fontWeight: FontWeight.bold)), backgroundColor: Colors.redAccent));
                          return;
                        }
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          SizedBox(width: 8),
                          Text(havuzVarMi ? "KAPORA İLE REZERVE ET" : "GÜVENCESİZ İLAN", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 0.5, fontFamily: 'Avenir')),
                        ],
                      ),
                    ),
                  ),
  ...
  Widget _buildDetayKutusu(IconData ikon, String baslik, String deger, Color cardColor) {
    return Container(
      ...
      child: Row(
        children: [
          Container(padding: EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.teal.shade700.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(ikon, color: Colors.teal.shade700, size: 18)),
          SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(baslik, style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'Avenir')), SizedBox(height: 2), Text(deger, style: TextStyle(color: Color(0xFF1E293B), fontSize: 13, fontWeight: FontWeight.w900, fontFamily: 'Avenir'))])),
        ],
      ),
    );
  }
