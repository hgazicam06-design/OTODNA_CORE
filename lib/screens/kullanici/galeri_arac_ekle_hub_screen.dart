...
class GaleriAracEkleHubScreen extends StatefulWidget {
  GaleriAracEkleHubScreen({super.key});
...
class _GaleriAracEkleHubScreenState extends State<GaleriAracEkleHubScreen> {
  ...
  void _hubSorgusuBaslat() async {
    ...
    if (girilenSase.length < 5) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Geçersiz Şase (VIN) numarası.", style: TextStyle(color: SiberTema.textMain, fontFamily: 'Avenir')), backgroundColor: Colors.redAccent));
      return;
    }
    ...
    if (hubRaporu != null && hubRaporu.containsKey("detaylar")) {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Merkez Veritabanı: Fabrika verileri eşleşti.", style: TextStyle(color: SiberTema.textMain, fontWeight: FontWeight.bold, fontFamily: 'Avenir')), backgroundColor: primaryTeal));
      }
    } else {
      // ARAÇ BULUNAMADIYSA
      if (mounted) {
        setState(() => _hubSorgulaniyor = false);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Sistem Hatası: Bu şase numarasına ait fabrika verisi bulunamadı.", style: TextStyle(color: SiberTema.textMain, fontFamily: 'Avenir')), backgroundColor: Colors.redAccent));
      }
    }
  }
  ...
  void _ilaniYayinla() {
    if (_fiyatController.text.isEmpty || _kmController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Lütfen Fiyat ve Kilometre bilgilerini girin.", style: TextStyle(color: SiberTema.textMain, fontFamily: 'Avenir')), backgroundColor: Colors.redAccent));
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Araç Kaydı Plaza Ağına Başarıyla Mühürlendi.", style: TextStyle(color: SiberTema.textMain, fontWeight: FontWeight.bold, fontFamily: 'Avenir')), backgroundColor: primaryTeal));
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _hubSorgulaniyor
                      ? Row(mainAxisAlignment: MainAxisAlignment.center, children: [SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: primaryTeal, strokeWidth: 2)), SizedBox(width: 12), Text("Veritabanı Taranıyor...", style: TextStyle(color: primaryTeal, fontWeight: FontWeight.w900, fontFamily: 'Avenir'))])
                      : Text("MERKEZDEN VERİLERİ ÇEK", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14, letterSpacing: 1, fontFamily: 'Avenir')),
                ),
              ),
              SizedBox(height: 40),
              ...
              if (_aracBulundu) ...[
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Container(padding: EdgeInsets.all(12), decoration: BoxDecoration(color: primaryTeal.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(Icons.verified, color: primaryTeal, size: 28)),
                          SizedBox(width: 16),
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                ...
                                SizedBox(height: 4),
                                Text("${_hubVerileri["Paket"]} - ${_hubVerileri["Yıl"]}", style: TextStyle(color: Colors.white54, fontSize: 13, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                              ],
                            ),
                          )
                        ],
                      ...
                      Padding(padding: EdgeInsets.symmetric(vertical: 20), child: Divider(color: Colors.white.withValues(alpha: 0.05), height: 1)),

                      GridView.count(
                        shrinkWrap: true, physics: NeverScrollableScrollPhysics(), crossAxisCount: 2, crossAxisSpacing: 16, mainAxisSpacing: 20, childAspectRatio: 2.5,
                        children: [
                ...
                InkWell(
                  onTap: () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Medya Yöneticisi Açılıyor..."), backgroundColor: primaryTeal)),
                  borderRadius: BorderRadius.circular(16),
                  child: Container(
                    width: double.infinity, padding: EdgeInsets.symmetric(vertical: 32),
                    ...
                    child: Column(
                      children: [
                        Container(padding: EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: primaryTeal.withValues(alpha: 0.1))), child: Icon(Icons.add_photo_alternate_outlined, color: primaryTeal, size: 36)),
                        SizedBox(height: 16),
                        Text("Araç Fotoğraflarını Yükle", style: TextStyle(color: textColor, fontWeight: FontWeight.w900, fontSize: 14, fontFamily: 'Avenir')),
                        SizedBox(height: 4),
                        Text("Maksimum 20 Fotoğraf", style: TextStyle(color: Colors.white54, fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                      ],
                    ),
                  ),
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text("SİSTEME KAYDET VE YAYINLA", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14, letterSpacing: 1, fontFamily: 'Avenir')),
                  ),
                ),
                SizedBox(height: 40),
              ]
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(baslik, style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
              SizedBox(height: 2),
  ...
  Widget _buildPremiumInput(String hint, IconData icon, TextEditingController controller, {bool isNumber = false, bool isUppercase = false}) {
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(prefixIcon: Icon(icon, color: primaryTeal, size: 20), hintText: hint, hintStyle: TextStyle(color: Colors.white38, fontSize: 13, fontFamily: 'Avenir'), border: InputBorder.none, contentPadding: EdgeInsets.symmetric(vertical: 16)),
      ),
    );
  }
}