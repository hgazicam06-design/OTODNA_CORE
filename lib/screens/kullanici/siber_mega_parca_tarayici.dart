...
class SiberMegaParcaTarayiciScreen extends StatefulWidget {
  SiberMegaParcaTarayiciScreen({super.key});
...
class _SiberMegaParcaTarayiciScreenState extends State<SiberMegaParcaTarayiciScreen> {
  ...
  void _rehberiGoster({bool otomatik = false}) {
    String baslik = "AKILLI PARÇA TARAYICI";
    String icerik = "Ustanın elinize verdiği yedek parça listesini kameraya okutun!\n\n"
  ...
  void _hurdaciOnayiIste() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Column(
          children: [
            Container(padding: EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.orange.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(Icons.recycling, color: Colors.orange.shade700, size: 48)),
            SizedBox(height: 16),
        ...
        content: Column(
          ...
          children: [
            Text("Ağımızda ${_bulunanParcalar.length} parçayı bulduk ve sepetinize ekledik. Ancak aşağıdaki ${_eksikParcalar.length} parça stoklarda yok:", textAlign: TextAlign.center, style: TextStyle(color: Colors.white54, fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
            SizedBox(height: 16),
            Container(
            ...
            SizedBox(height: 16),
            Text("İzninizle bu parçalar için yetkili tedarikçilerimizden anlık ihale teklifi isteyelim mi?", textAlign: TextAlign.center, style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
          ],
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: Text("İSTEMİYORUM", style: TextStyle(color: Colors.white38, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
          ),
          ElevatedButton(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Tedarikçi İhalesi Başlatıldı! Teklifler Cüzdanınıza düşecek.", style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Avenir')), backgroundColor: Colors.orange.shade700));
            },
            child: Text("EVET, TEKLİF TOPLA", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 0.5, fontFamily: 'Avenir')),
          )
        ],
      ),
    );
        ...
        body: Column(
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  Text("LİSTEYİ YAPAY ZEKAYA OKUTUN", style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.5, fontFamily: 'Avenir')),
                  SizedBox(height: 16),
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      icon: _tariyor ? SizedBox(width: 24, height: 24, child: CircularProgressIndicator(color: Colors.white)) : Icon(Icons.document_scanner, size: 24, color: Colors.white),
                      label: Text(_tariyor ? "YAPAY ZEKA OKUYOR..." : "KAMERAYI AÇ VE TARA", style: TextStyle(fontSize: 13, fontWeight: FontWeight.w900, letterSpacing: 1, color: SiberTema.textMain, fontFamily: 'Avenir')),
                    ),
                  ),
                ],
              ),
                  ...
                  : Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 16),
                          Text("Taranan parçalar burada listelenecektir.", style: TextStyle(color: Colors.white38, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                        ],
                      ),
                    ),
            ),
            ...
            if (_taramaBitti && _bulunanParcalar.isNotEmpty)
              Container(
                padding: EdgeInsets.all(24),
                decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.white.withValues(alpha: 0.05), blurRadius: 20, offset: Offset(0, -5))]),
                child: SafeArea(
                  child: Row(
                    ...
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text("TOPLAM SEPET", style: TextStyle(color: Colors.white54, fontSize: 10, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
                      ...
                      ElevatedButton(
                        style: ElevatedButton.styleFrom(backgroundColor: primaryTeal, foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), padding: EdgeInsets.symmetric(horizontal: 24, vertical: 16)),
                        onPressed: () {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Siparişiniz satıcılara iletildi!", style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Avenir')), backgroundColor: primaryTeal));
                        },
                        child: Text("SİPARİŞİ ONAYLA", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 0.5, fontFamily: 'Avenir')),
                      )
                    ],
                  ),
                ),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
          ...
          child: Row(
            children: [
              Container(padding: EdgeInsets.all(12), decoration: BoxDecoration(color: badgeColor.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(isCikma ? Icons.recycling : Icons.build_circle, color: badgeColor, size: 24)),
              SizedBox(width: 16),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text("Satıcı: ${item['bayi_adi']}", style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                    SizedBox(height: 8),
                    Container(
