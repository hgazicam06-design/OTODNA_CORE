...
class AracDnaRaporuScreen extends StatefulWidget {
  ...
  AracDnaRaporuScreen({super.key, required this.saseNo});
...
class _AracDnaRaporuScreenState extends State<AracDnaRaporuScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _glowController = AnimationController(vsync: this, duration: Duration(seconds: 3))..repeat(reverse: true);
    _siberMuhurUret();
    _aracVerisiniCek();
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text("Bu aracın tüm genetik sicilini (Yağ bakımından fren balatasına, tramerden ekspertiz raporuna kadar) içeren Değiştirilemez PDF Raporunu almak üzeresiniz.", style: TextStyle(color: SiberTema.textMuted, fontSize: 12, height: 1.5)),
            SizedBox(height: 16),
            Container(
              ...
              child: Text("KİŞİSEL VERİLERİN KORUNMASI GEREĞİ: Bu raporun üretilebilmesi için aracın yasal sahibine bir 'Veri Paylaşım Onayı' bildirimi gönderilecektir.", style: TextStyle(color: SiberTema.kanKirmizi, fontSize: 10, fontWeight: FontWeight.bold)),
            ),
            SizedBox(height: 16),
            Text("HİZMET BEDELİ: 599 ₺", style: TextStyle(color: SiberTema.kuantumCyan, fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: 1)),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Araç Sahibine Onay İsteği Gönderildi! ⏳", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), backgroundColor: SiberTema.altinSari));
              
              // Simülasyon: 3 Saniye sonra onaylandı varsayalım
              await Future.delayed(Duration(seconds: 3));
              if (!mounted) return;
              
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Araç Sahibi ONAYLADI. Cüzdandan 599 ₺ çekildi. Rapor İndiriliyor! 📥", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), backgroundColor: SiberTema.kuantumCyan, duration: Duration(seconds: 4)));
            },
            child: Text("ONAY İSTE (599 ₺)", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900)),
          )
        ],
      ),
    );
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      Text("${_aracVerisi?['plaka']} - ${_aracVerisi?['marka_model']}".toUpperCase(), style: TextStyle(color: SiberTema.textMain, fontSize: 16, fontWeight: FontWeight.w900, letterSpacing: 1)),
                      SizedBox(height: 8),
                      Text("ŞASE: ${widget.saseNo.toUpperCase()}", style: TextStyle(color: SiberTema.textMuted, fontSize: 11, letterSpacing: 2)),
                      Padding(padding: EdgeInsets.symmetric(vertical: 12), child: Divider(color: SiberTema.textMuted)),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.fingerprint, color: SiberTema.kuantumCyan, size: 16),
                          SizedBox(width: 8),
                          Text("SİBER MÜHÜR: $_siberMuhur", style: TextStyle(color: SiberTema.kuantumCyan, fontSize: 10, fontFamily: 'monospace', fontWeight: FontWeight.bold, letterSpacing: 1.5)),
                        ],
                      )
                    ],
                  ),
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 8),
                          Text("GENEL DNA SAĞLIK SKORU", style: TextStyle(color: SiberTema.textMain, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 2)),
                        ],
                      ),
                    );
                  },
                ...
                // =================================================================
                Align(alignment: Alignment.centerLeft, child: Text("KAYITLI SERVİS & BAKIM GEÇMİŞİ", style: TextStyle(color: SiberTema.textMuted, fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.5))),
                SizedBox(height: 16),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    icon: Icon(Icons.picture_as_pdf, size: 24, color: Colors.white),
                    label: Text("TÜM SİCİLİ AL (599 ₺)", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, letterSpacing: 1, fontSize: 15)),
                  ),
                ),
                SizedBox(height: 20),
                Text("Bu rapor OtoDNA Kuantum Ağı tarafından kriptolanmıştır. Veri İzinsiz Çoğaltılamaz.", textAlign: TextAlign.center, style: TextStyle(color: SiberTema.textMuted, fontSize: 9)),
                SizedBox(height: 40),
              ],
            ),
          ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(parca, style: TextStyle(color: SiberTema.textMain, fontSize: 13, fontWeight: FontWeight.bold)),
                SizedBox(height: 4),
                Text(islem, style: TextStyle(color: islemRengi, fontSize: 11, fontWeight: FontWeight.w900)),
              ],
            ),
          ),
          Text(zaman, style: TextStyle(color: SiberTema.textMuted, fontSize: 10, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
