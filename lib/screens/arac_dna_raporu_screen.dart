...
class _AracDnaRaporuScreenState extends State<AracDnaRaporuScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    // 3 Sayfalık Siber DNA Kitapçığı
    ...
    return ResponsiveKalkan(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text("ULTRA DNA KİTAPÇIĞI", style: TextStyle(color: Colors.white.withOpacity(0.9), fontWeight: FontWeight.w900, fontSize: 14, letterSpacing: 1.5, fontFamily: 'Avenir')),
          centerTitle: true,
          bottom: TabBar(
            ...
            tabs: const [
              Tab(text: "SİCİL & KİMLİK", icon: Icon(Icons.fingerprint)),
              Tab(text: "EKSPERTİZ", icon: Icon(Icons.analytics)),
              Tab(text: "REVİZYON", icon: Icon(Icons.history)),
            ],
          ),
        ),
        body: Container(
          ...
          child: TabBarView(
            ...
            children: [
              _buildSicilVeKimlikSayfasi(), // Sayfa 1
              _buildEkspertizSayfasi(),     // Sayfa 2
              _buildRevizyonSayfasi(),      // Sayfa 3
            ],
          ),
        ),
      ),
  ...
  // ── SAYFA 1: SİCİL VE KİMLİK (Röntgen & Seyahat Geçmişi) ──
      ...
      builder: (context, snapshot) {
        ...
        return ListView(
          ...
          children: [
              ...
              child: Column(
                children: [
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(widget.plaka, style: const TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w900, letterSpacing: 2, fontFamily: 'Avenir')),
                            const SizedBox(height: 4),
                            Text(aracData['model'] ?? 'Bilinmeyen Model', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 14, fontFamily: 'Avenir')),
                            const SizedBox(height: 8),
                            Text("ŞASE: $saseNo", style: const TextStyle(color: SiberTema.altinSari, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.5, fontFamily: 'Avenir')),
                          ],
                        ),
                      ),
                      Container(
                        ...
                        child: Column(
                          children: [
                            Text(dnaSkoru.toStringAsFixed(0), style: TextStyle(color: skorRengi, fontSize: 24, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
                            Text("DNA", style: TextStyle(color: skorRengi.withOpacity(0.8), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1)),
                          ],
                        ),
                      ),
                    ],
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: isRiskli ? SiberTema.kanKirmizi.withOpacity(0.2) : SiberTema.kuantumCyan.withOpacity(0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: isRiskli ? SiberTema.kanKirmizi : SiberTema.kuantumCyan.withOpacity(0.3))),
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(isRiskli ? "TRAFİĞE ÇIKIŞI RİSKLİ" : "KARARGAH ONAYLI", style: TextStyle(color: isRiskli ? SiberTema.kanKirmizi : SiberTema.kuantumCyan, fontWeight: FontWeight.w900, fontSize: 12, letterSpacing: 1, fontFamily: 'Avenir')),
                      ],
                    ),
                  )
                ],
            ...
            // SEYAHAT VE PLAKA DEĞİŞİM GEÇMİŞİ (Siber İstihbarat)
              ...
              builder: (context, gecmisSnapshot) {
                ...
                if (gecmisList.isEmpty) {
                  return Container(
                    ...
                    child: Text("Karargah kayıtlarında henüz bir plaka/il değişikliği bulunmuyor.", style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 12, fontFamily: 'Avenir')),
                  );
                }
                ...
                return Column(
                  children: gecmisList.map((doc) {
                    ...
                    String tur = islem['islem_turu'] ?? 'Kayıt'; // Örn: "İl Değişikliği", "Plaka Değişikliği"
                    String detay = islem['detay'] ?? ''; // Örn: "06 ABC -> 34 XYZ" veya "Ankara -> İstanbul"
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(tur, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12, fontFamily: 'Avenir')),
                                const SizedBox(height: 4),
                                Text(detay, style: TextStyle(color: SiberTema.altinSari.withOpacity(0.8), fontWeight: FontWeight.w900, fontSize: 14, fontFamily: 'Avenir', letterSpacing: 1)),
                              ],
                            ),
                          ),
                          Text(tarihStr, style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 10, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                        ],
                      ),
                    );
                  }).toList(),
  ...
  // ── SAYFA 2: DETAYLI EKSPERTİZ TESTLERİ (Fren, Motor, Kaporta) ──
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return _buildSiberUyari("ARACA AİT DETAYLI EKSPERTİZ RAPORU BULUNMUYOR.");
        }

        var rapor = snapshot.data!.docs.first.data() as Map<String, dynamic>;

        // Örnek Veri Çekimleri
        ...
        List<dynamic> kaportaListesi = rapor['kaporta_degisenler'] ?? []; // Örn: ["Sol Ön Çamurluk (Değişen)", "Sağ Kapı (Boyalı)"]
            ...
            kaportaListesi.isEmpty
              ...
              child: const Row(
                children: [
                  ...
                  SizedBox(width: 12),
                  Expanded(child: Text("Orijinal: Boya veya Değişen tespit edilmedi.", style: TextStyle(color: SiberTema.kuantumCyan, fontWeight: FontWeight.bold, fontFamily: 'Avenir', fontSize: 12))),
                ],
              ),
            )
                : Column(
                ...
                return Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(parca.toString(), style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12, fontFamily: 'Avenir')),
                    ],
                  ),
                );
              }).toList(),
  ...
  // Test Barlarını (Progress) Oluşturan Yardımcı Widget
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(baslik, style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
              Text("%${yuzde.toInt()}", style: TextStyle(color: renk, fontSize: 14, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
            ],
          ),
          const SizedBox(height: 8),
          ClipRRect(
  ...
  // ── SAYFA 3: MEGA REVİZYON ZAMAN ÇİZELGESİ (Usta Kayıtları) ──
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(islemTarihi, style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 10, fontWeight: FontWeight.bold, fontFamily: 'Avenir')),
                              Text("₺${maliyet.toStringAsFixed(2)}", style: const TextStyle(color: SiberTema.kuantumCyan, fontSize: 14, fontWeight: FontWeight.w900, fontFamily: 'Avenir')),
                            ],
                          ),
                          const SizedBox(height: 12),
                          ...parcalar.map((p) {
                            var parcaData = p as Map<String, dynamic>;
                            String pAdi = parcaData['parca_adi'] ?? 'Parça';
                            String gorsel = parcaData['gorsel_url'] ?? '';
                            return Padding(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  Expanded(child: Text(pAdi, style: TextStyle(color: Colors.white.withOpacity(0.9), fontSize: 12, fontFamily: 'Avenir'))),
                                  if (gorsel.isNotEmpty) const Icon(Icons.image, color: SiberTema.altinSari, size: 14)
                                ],
                              ),
                            );
                          }).toList(),
  ...
  // --- Ortak Başlık Widget'ı ---
  Widget _buildPanelBaslik(String baslik, IconData ikon) {
    return Row(
      children: [
        Icon(ikon, color: SiberTema.kuantumCyan, size: 18),
        const SizedBox(width: 8),
        Text(baslik, style: TextStyle(color: Colors.white.withOpacity(0.7), fontWeight: FontWeight.w800, letterSpacing: 1.5, fontSize: 11, fontFamily: 'Avenir')),
      ],
    );
  }

  // --- Ortak Uyarı Widget'ı ---
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(mesaj, textAlign: TextAlign.center, style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.5, fontFamily: 'Avenir')),
          ],
        ),
      ),
    );
