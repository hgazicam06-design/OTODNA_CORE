...
class VehicleService {
  ...
  Future<VehiclePublicInfo?> getPublicInfo(String saseNo) async {
    try {
      ...
      final doc = await _db.collection('araclar').doc(muhurluSase).get();

      if (!doc.exists) {
        developer.log("SİBER İHLAL: Radarda böyle bir araç (DNA) bulunamadı!");
        // Bu bir hata değildir, araç gerçekten yoktur. Bu yüzden null dönmek doğrudur.
        return null;
      }
      ...
      // 🚨 SESSİZ ÇÖKÜŞ ENGELLENDİ: Ağ koparsa UI'a yalan söyleme, hata fırlat!
      throw Exception("SİBER RADAR HATASI: Kuantum Ağına bağlanılamıyor, araç kimliği çekilemedi!");
    }
  }
  ...
  Future<void> engelleIp(String saseNo, String ip) async {
    try {
      ...
      throw Exception("SİBER SAVUNMA HATASI: Saldırgan IP adresi ağa mühürlenemedi. Bağlantıyı kontrol edin!");
    }
  }
  ...
  Future<void> engelKaldir(String saseNo, String ip) async {
    try {
      ...
      // 🚨 SESSİZ ÇÖKÜŞ ENGELLENDİ
      throw Exception("SİBER SAVUNMA HATASI: IP engeli kaldırılamadı, Kuantum Ağına ulaşılamıyor!");
    }
  }
  ...
  Future<void> okunduIsaretle(String saseNo, String bildirimId) async {
    try {
      ...
      // 🚨 SESSİZ ÇÖKÜŞ ENGELLENDİ
      throw Exception("İLETİŞİM HATASI: Sinyal 'okundu' olarak işaretlenemedi!");
    }
  }
}